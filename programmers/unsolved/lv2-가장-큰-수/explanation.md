...
### [6, 10, 2] 상세 비교 과정
`numbers = [6, 10, 2]`인 경우, 문자열 리스트는 `['6', '10', '2']`가 됩니다.
정렬 기준 `key=lambda x: x*3`을 적용하면 다음과 같이 비교됩니다.
...
2. **문자열 비교 (사전 순)**
    - **"666" vs "101010"**: 앞글자 '6' > '1' 이므로 "666"이 더 큼.
    - **"666" vs "222"**: 앞글자 '6' > '2' 이므로 "666"이 더 큼.
    - **"222" vs "101010"**: 앞글자 '2' > '1' 이므로 "222"가 더 큼.

3. **정렬 결과 (내림차순)**
    - 순서: `'6'`("666") > `'2'`("222") > `'10'`("101010")
    - 결과 리스트: `['6', '2', '10']`

4. **최종 결과**: 이어 붙여서 `"6210"` 반환
