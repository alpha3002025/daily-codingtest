...
### 짝수 왕복이 아닐 경우(홀수 거리)를 필터링하는 이유는 무엇인가요?

한 번 움직일 때마다 목적지까지의 거리는 **반드시 홀/짝 성질이 바뀝니다.**
그래서 **남은 턴 수(`limit`)**와 **최단 거리(`dist`)**의 홀짝성이 일치하지 않으면 **절대 도착할 수 없습니다.**

#### 예시 상황
-   **목표까지 남은 거리**: **1칸** (홀수)
-   **남은 턴 수**: **4번** (짝수)
-   **목표**: 4번 움직여서 1칸 떨어진 곳에 도착해야 함.

#### 시뮬레이션
1.  **1번 이동** (거리 0, 남은 턴 3): 도착했지만 턴이 남음 -> 더 움직여야 함.
2.  **2번 이동** (거리 1, 남은 턴 2): 목표에서 멀어짐.
3.  **3번 이동** (거리 0, 남은 턴 1): 다시 도착했지만 턴이 1 남음.
4.  **4번 이동** (거리 1, 남은 턴 0): 턴 종료. 하지만 목표에서 1칸 떨어져 있음 -> **실패!**

#### 결론
"갔다 오기(왕복)"는 항상 2턴(짝수)을 소모합니다.
최단 거리가 1(홀수)이라면, 도착 가능한 턴 수는 `1, 3, 5, 7...` 처럼 무조건 **홀수**여야 합니다.
따라서 **남은 턴이 짝수라면(4번)**, 물리적으로 도착이 불가능하므로 미리 포기(가지치기)하는 것입니다.
