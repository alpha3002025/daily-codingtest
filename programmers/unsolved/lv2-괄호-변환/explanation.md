# 괄호 변환
- 재귀 문제다. 문제에서 원하는 절차를 그대로 구현해야 한다.
- `str.maketrans(src, dest)`, `translate(table)`과 같은 문자열 함수를 사용하는 연습도 해보자. 물론 하드코딩 역시 가능하지만, 이런 함수 몇번 연습해보는 것도 괜찮다고 본다.
- 겉으로 한 번 봤을때는 '재귀' 로직이 잘 안보이지만, 계속 풀어보거나,  코드를 자세히 보면 '재귀'성격이 매우 강하다. 
- 괄호 검사는 stack 으로 유효한 괄호 문자열인지 체크하는 것은 굉장히 유명한 개념이다. 이 개념이 재귀와 섞어서 나왔다. 문제가 꽤 현란하게 콤비네이션을 썼구나 하고 생각했다.
<br/>
