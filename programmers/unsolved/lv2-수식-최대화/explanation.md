...
## Q&A
### Question 1: [ *, -, +] 가 operators의 현재 상태일 때, 우선순위 순열이 [-, *, +] 라면 idx는 어떻게 변하나요?

`operators_copy`가 `['*', '-', '+']`이고, 우선순위 순열이 `['-', '*', '+']`일 때의 `idx` 변화를 연산자별로 나누어 단계별로 설명해 드리겠습니다.

#### 1단계: 우선순위 연산자가 `-` 일 때
(현재 리스트: `['*', '-', '+']`, 목표: `-`)

1.  **idx = 0**
    -   `operators[0]`은 `*`입니다. (`-` 아님)
    -   `else` 블록 → **`idx` 1 증가**
    -   **현재 idx: 1**
2.  **idx = 1**
    -   `operators[1]`은 `-`입니다. (**일치!**)
    -   연산 수행 후 `del` 실행.
    -   리스트 변화: `['*', '-', '+']` → `['*', '+']`
    -   뒤에 있던 `+`가 인덱스 1로 당겨짐.
    -   **`idx` 유지 (증가 안 함)**
    -   **현재 idx: 1**
3.  **idx = 1 (재검사)**
    -   `operators[1]`은 `+`입니다. (`-` 아님)
    -   `else` 블록 → **`idx` 1 증가**
    -   **현재 idx: 2**
4.  **루프 종료**
    -   `idx(2) < len(2)` 조건 거짓으로 종료.
