...
> **상세 설명 (55의 경우)**:
> 1. **올림 선택 (55 -> 60)**: 
>    - 일의 자리 5를 올림 (+5회) $\rightarrow$ 60
       - -> 위로 1x5층 올라가서 60층 (+5)
>    - 십의 자리 6을 올림 (+4회) $\rightarrow$ 100
       - -> 위로 10x4층 올라가서 100층 (+4)
>    - 백의 자리 1을 내림 (+1회) $\rightarrow$ 0
       - -> 아래로 100x1층 내려가서 0층 (+1)
>    - **총 10회** ($5+4+1$)
> 2. **내림 선택 (55 -> 50)**:
>    - 일의 자리 5를 내림 (+5회) $\rightarrow$ 50
>    - 십의 자리 5를 올림/내림
>      - (올림 시): 50 $\rightarrow$ 100 (+5회) $\rightarrow$ 0 (+1회) = **총 11회** ($5+5+1$)
>      - (내림 시): 50 $\rightarrow$ 0 (+5회) = **총 10회** ($5+5$)
> 
> *잠깐, 55는 둘 다 10회 아닌가요?* 
> 아, 위 예시는 조금 헷갈릴 수 있겠네요. **65**나 **95** 같은 경우가 더 명확합니다.
> `55`의 경우, 올리면 10회, 내리면 11회(50->100->0) 혹은 10회(50->0)가 됩니다.
> 코드는 "5 이상"일 때 올리도록 되어 있어 55는 60으로 만들게 됩니다.
> 핵심은 **앞자리가 5 이상이면, 지금 올려줘서 앞자리를 6 이상으로 만들면, 앞자리도 '올림' 처리(4번 클릭)를 받을 수 있어 이득**이라는 점입니다.
