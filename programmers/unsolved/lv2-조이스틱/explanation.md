...
### Q2. `next_i`와 `n - next_i`의 의미는 무엇인가요?

**1. `next_i`**
- **의미**: 현재 위치(`i`) 이후, **연속된 'A'를 건너뛰고 만나는 첫 번째 'A가 아닌 문자'의 인덱스**입니다.
- 즉, **"앞에서부터 셌을 때 다음 타겟의 위치"**입니다.
- **예시**: `BBBAAZ` (길이 6)
    - `i=2` ('B') 이후에 'A', 'A'가 나오므로 건너뛰고, `next_i`는 **5** ('Z')가 됩니다.

- **"모든 위치 i에서 시작하여, 만약 바로 뒤에 연속된 'A'들이 있다면 그것들을 한꺼번에 건너뛴 후의 위치(next_i)**

<br/>

**2. `n - next_i`**
- **의미**: **원점(0)에서 왼쪽(뒤로) 이동하여 `next_i`에 도달하기 위한 거리**입니다.
- 즉, **"뒤에서부터 셌을 때 다음 타겟까지의 거리"**입니다.
- **예시**: `BBBAAZ`
    - 전체 길이 `n=6`, `next_i=5`
    - `6 - 5 = 1`
    - 원점(0)에서 왼쪽 키를 한 번 누르면 인덱스 5('Z')에 도달하므로 거리는 **1**입니다.

<br/>

### Q3. `BBBAAZ`일 때 `next_i`가 5가 되는 과정은?
...
**탐색 과정 (While Loop)**:
1.  **초기화**: `next_i = i + 1 = 3`. (`name[3]`은 'A')
2.  **첫 번째 반복**:
    -   조건: `next_i < 6` (True) **AND** `name[3] == 'A'` (True)
    -   실행: `next_i += 1` → **4**
3.  **두 번째 반복**:
    -   조건: `next_i < 6` (True) **AND** `name[4] == 'A'` (True)
    -   실행: `next_i += 1` → **5**
4.  **세 번째 반복**:
    -   조건: `next_i < 6` (True) **AND** `name[5] == 'A'` ('Z'이므로 False)
    -   **종료**

**결과**:
- `next_i`는 **5**가 됩니다.
- 즉, 연속된 'A'들을 모두 건너뛰고 **다음에 처리해야 할 문자인 'Z'(인덱스 5)의 위치**를 정확히 찾았습니다.

<br/>

### Q4. `i * 2 + (n - next_i)` (오른쪽으로 갔다가 돌아오기)의 의미는 무엇인가요?
...
### Q5. `(n - next_i) * 2 + i` (왼쪽으로 갔다가 돌아오기)의 의미는 무엇인가요?
